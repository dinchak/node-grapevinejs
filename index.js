/**
 * Integration with the Grapevine inter-MUD communication protocol.
 * 
...
const debug = require('debug')('grapevine')
...
/**
 * Array of games discovered on the grapevine network
 * @type {Array.<string>}
 */
let games = []
 ...
 * @type {string}
 */
let url = 'wss://grapevine.haus/socket'
...
/**
 * Initialize the grapevine connection.  client_id and client_secret are required, but the
 * rest have defaults.  Configuration options are as follows:
 *   
 *   client_id: Your grapevine client_id
 *   client_secret: Your grapevine client_secret
 ...
 *   url: grapevine websocket url
 *   
 * See https://grapevine.haus/docs#authenticate for additional options.
 * 
...
/**
 * Sends an event to grapevine.  See https://grapevine.haus/docs for more info.  Example:
 *
 *   let payload = await send('channels/subscribe', {channel: 'grapevine'})
 * 
...
/**
 * Handles an incoming message from grapevine.
 * 
...
/**
 * Add a player and announce it to the grapevine network.
 * 
...
/**
 * Remove a player and announce it to the grapevine network.
 * 
 * @param {string} name The player name
...
/**
 * True if the connection to grapevine is open and authenticated.
 * @returns {boolean} true if the connection to grapevine is available
 */
function isAlive() {
  return alive
}
